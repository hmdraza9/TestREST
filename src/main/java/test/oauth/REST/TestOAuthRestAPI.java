...
public class TestOAuthRestAPI {
	...
	public String getOAuthCode() throws IOException {
		...
		getCodeURI.append(authURL).append("?").append("scope=").append(scope).append("&auth_url=").append(auth_url)
				.append("&client_id=").append(client_id).append("&response_type=").append(response_type)
		...
		try {
			...
			// TakesScreenshot scr = (TakesScreenshot)
			// driver;Assert.assertTrue(accessTokenURL, false);
...
//		.all();
		...
		response = given().queryParam("access_token", token).expect().defaultParser(Parser.JSON).when()
